...
from flask_app.models import sighting # Need to import the pet FILE not the class
...
class User:
    ...
    def __init__(self, data):
        ...
        self.sightings = [] # This is the to many part of the one to many 
    ...
    def get_one_join(cls, data):
        ...
        query = "SELECT * FROM users LEFT JOIN sightings ON users.id = sightings.user_id WHERE users.id = %(id)s;" # MAKE SURE THE LEFT TABLE IS plural
        ...
        if results[0] != None:
            for row in results:
                ...
                user.sightings.append(sighting.Sighting(row_data)) # append to the specific user list
