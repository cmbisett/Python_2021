...
from flask_app.models import recipe # Need to import the pet FILE not the class
...
class User:
    ...
    def get_one_join(cls, data):
        ...
        query = "SELECT * FROM users LEFT JOIN recipes ON users.id = recipes.user_id WHERE users.id = %(id)s;" # MAKE SURE THE LEFT TABLE IS plural
        ...
        if results[0]['recipes.id'] != None:
            for row in results:
                ...
                user.recipes.append(pet.Pet(row_data)) # append to the specific user list
    ...
    def validate_registration(form_data): #validation is the process of sanitizing the input from the user taken from the form
        ...
        if not EMAIL_REGEX.match(form_data['email']): #if the email is not formatted correctly
            ...
            is_valid = False 
        elif User.get_by_email({"email": form_data['email']}): #if the email already exists within the database
